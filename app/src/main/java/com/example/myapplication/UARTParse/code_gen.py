...
def gen_java(output_file):
    ...
    with open(output_file, "w") as f:
        if (config["PACKAGE"] != ""):
            ...
            f.write("package UARTParse;\n")
        f.write("import java.util.Map;\n")
        f.write("import java.util.HashMap;\n")
        f.write("import java.util.Collections;\n")
        f.write("public class Sensors_CFG {\n")
        f.write(' '*4 + f'public final static int HEADER_SIZE_BITS = {config["Header"]};\n')
        f.write(' '*4 + f'public final static int HEADER_SIZE_BYTES = {config["Header"]//8};\n')
        f.write(' '*4 + f'public final static int MAX_DATA_SIZE_BITS = {bit_ct};\n')
        f.write(' '*4 + f'public final static int MAX_DATA_SIZE_BYTES = {bit_ct//8 + int(bit_ct%8!=0)};\n')
        f.write(' '*4 + f'public final static int CRC_SIZE_BITS = {config["CRC"]};\n')
        f.write(' '*4 + f'public final static int CRC_SIZE_BYTES = {config["CRC"]//8};\n')
        f.write(' '*4 + 'public final static String[] ordered_sensors = new String[]{')
        i = 0
